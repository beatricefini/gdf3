document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    ...
    instructions.addEventListener("click", async () => {
      ...
      // --- Start Drag/Models Main ---
      startDragMain();
    }, { once: true });

  }, 5000);

  // --- Funzione principale del drag puzzle ---
  window.startDragMain = function() {
    ...
    const pieces = [];
    const positions = [
      { x:-0.2,y:0,z:0 }, { x:-0.5,y:0.6,z:0 }, { x:0.2,y:0,z:0 },
      { x:0.15,y:-0.5,z:0 }, { x:0.15,y:-0.45,z:0 }, { x:-0.1,y:0.3,z:0 }
    ];
    ...
    function checkSnap(piece){
      ...
      const dist = Math.sqrt((pos.x-centerPos.x)**2 + (pos.y-centerPos.y)**2 + (pos.z-centerPos.z)**2);
      if(dist<raggioSnap){
    ...
    function onPointerDown(event){
      ...
      const intersects = raycaster.intersectObjects(pieces.filter(p=>p.dataset.locked==="false").map(p=>p.object3D), true);
    ...
    function onPointerMove(event){
      ...
      const lerp = 0.15;
      currentPos.x += (targetPos.x - currentPos.x)*lerp;
      currentPos.y += (targetPos.y - currentPos.y)*lerp;
      currentPos.z = 0;
      selectedPiece.setAttribute('position',currentPos);
      checkSnap(selectedPiece);

      if(pieces.every(p=>p.dataset.locked==='true')){
        const t = document.getElementById('dragText');
