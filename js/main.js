...
window.startDragMain = () => {
  ...
  function onPointerMove(event){
    ...
    if(pieces.every(p => p.dataset.locked==='true')){
      ...
      pieces.forEach(p => { if(p.parentNode) p.parentNode.remove(); });
      ...
      setTimeout(() => {
        ...
        finalShape.setAttribute('animation__move', { property:'position', to:`${finalPos.x} ${finalPos.y} ${finalPos.z}`, dur:1000, easing:'easeInOutQuad' });
        finalShape.setAttribute('animation__scale', { property:'scale', to:'0.1 0.1 0.1', dur:1000, easing:'easeInOutQuad' });

        // MODELLO CINEMA
        const cinemaModel = document.createElement('a-entity');
        cinemaModel.setAttribute('gltf-model', '#pieceCinema');
        cinemaModel.setAttribute('position', { x:0.05, y:-0.25, z:0 });
        ...
        text1960.setAttribute('position',{ x:0, y:0.25, z:0 });
        text1960.setAttribute('scale','0.35 0.35 0.35');
        text1960.setAttribute('opacity','0');
        text1960.setAttribute('shader','msdf');
        text1960.setAttribute('animation__fadein',{ property:'opacity', from:0, to:1, dur:800, easing:'easeInQuad', delay:200 });
        ...
        textSculpture.setAttribute('position',{ x:0, y:0.1, z:0 });
        ...
        textSculpture.setAttribute('animation__fadein',{ property:'opacity', from:0, to:1, dur:800, easing:'easeInQuad', delay:1200 });
  ...
  // --- Pop-up animazione dopo TARGET FOUND ---
